...
SCHEMES = [
 ...
 {"id":"rental-arbitrage","title":"How to get rich renting out an apartment you don't own","steps":[
   {"cap":"Rent out a place you don't own.","vo":"Here's how to get rich from an apartment you don't even own.","layout":"mascot"},
   {"cap":"Rent a flat for $1,500.","vo":"First, you rent a totally normal apartment for fifteen hundred dollars a month.","layout":"icon","icon":"house","num":"$1,500","col":"red"},
   {"cap":"Sign a normal lease.","vo":"You sign a regular one year lease, nothing suspicious yet.","layout":"icon","icon":"house"},
   {"cap":"List it on Airbnb.","vo":"Then you quietly list that exact apartment on Airbnb, night by night.","layout":"icon","icon":"phone"},
   {"cap":"Tourists pay $120/night.","vo":"Tourists happily pay a hundred and twenty dollars for a single night.","layout":"icon","icon":"cash","num":"$120/night","col":"green"},
   {"cap":"20 nights = $2,400.","vo":"Rent it out just twenty nights a month, and that's twenty four hundred dollars.","layout":"equation","eq":"20 × $120","num":"= $2,400","col":"green"},
   {"cap":"Pay the landlord $1,500.","vo":"You pay your landlord their fifteen hundred and quietly keep everything above it.","layout":"two","a":"house","b":"cash"},
   {"cap":"Keep $900 a month.","vo":"That's nine hundred dollars a month on an apartment that isn't even yours.","layout":"icon","icon":"bag","num":"$900/mo","col":"green"},
   {"cap":"Now do it 10 times.","vo":"Now go and do the exact same thing with ten more apartments.","layout":"icons","icon":"house","num":"×10","col":"ink"},
   {"cap":"You own nothing. You're rich.","vo":"Congratulations, you are now a landlord who owns absolutely nothing.","layout":"mascot"},
 ]},

 {"id":"stoozing","title":"Banks will literally pay you to borrow money you never spend","steps":[
   {"cap":"Get paid to borrow money.","vo":"Here's how banks accidentally pay you just for borrowing money.","layout":"mascot"},
   {"cap":"Get a 0% APR card.","vo":"Get a credit card with zero percent interest for a whole year.","layout":"icon","icon":"card","num":"0% APR","col":"green"},
   {"cap":"Spend $10,000 on it.","vo":"Put ten thousand dollars of your normal spending on that card.","layout":"icon","icon":"cash","num":"$10,000","col":"red"},
   {"cap":"Keep YOUR $10,000.","vo":"Here's the trick, you keep your own ten thousand dollars in cash.","layout":"icon","icon":"bag"},
   {"cap":"Park it at 4% interest.","vo":"You park that cash in a savings account paying four percent interest.","layout":"icon","icon":"chart","num":"4% APY","col":"green"},
   {"cap":"The bank pays you $400.","vo":"Over the year, the bank pays you four hundred dollars just for keeping it there.","layout":"icon","icon":"cash","num":"$400","col":"green"},
   {"cap":"Pay the card off last-minute.","vo":"Right before the interest kicks in, you pay the card off in full.","layout":"two","a":"card","b":"cash"},
   {"cap":"$400 of the bank's money.","vo":"You just pocketed four hundred dollars of the bank's own money.","layout":"icon","icon":"bag","num":"$400 free","col":"green"},
   {"cap":"Do it with 5 cards.","vo":"Now run that exact move with five different cards at the same time.","layout":"icons","icon":"card","num":"×5","col":"ink"},
   {"cap":"They paid you to hold it.","vo":"They literally paid you just to hold onto their money.","layout":"mascot"},
 ]},

 {"id":"return-pallets","title":"Amazon sells its returns for pennies — here is the flip","steps":[
   {"cap":"Buy Amazon's returns for pennies.","vo":"Here's how to buy Amazon's own products for pennies on the dollar.","layout":"mascot"},
   {"cap":"Returns don't get restocked.","vo":"When people return things to Amazon, most of it never gets restocked.","layout":"icon","icon":"order"},
   {"cap":"They dump them on pallets.","vo":"Instead, all of it gets dumped onto giant returns pallets.","layout":"icons","icon":"order","num":"1 pallet","col":"ink"},
   {"cap":"A pallet costs $100.","vo":"You can buy an entire pallet, sight unseen, for about a hundred dollars.","layout":"icon","icon":"cash","num":"$100","col":"red"},
   {"cap":"200 random products inside.","vo":"Inside are a couple hundred completely random returned products.","layout":"icons","icon":"gift","num":"×200","col":"ink"},
   {"cap":"Worth $500+ resold.","vo":"Listed back online one by one, that stuff is worth five hundred dollars or more.","layout":"icon","icon":"chart","num":"$500+","col":"green"},
   {"cap":"That's 5x your money.","vo":"That is five times your money out of a single mystery box.","layout":"icon","icon":"bag","num":"5x","col":"green"},
   {"cap":"Buy 10 pallets a week.","vo":"So you buy ten of these pallets every single week.","layout":"loop","loop":["order","gift","chart","cash"]},
   {"cap":"Your garage is a warehouse.","vo":"Your garage is technically a warehouse now, but you are rich.","layout":"mascot"},
 ]},

 {"id":"scratch-roll","title":"Buy the whole roll of scratch tickets and the odds flip","steps":[
   {"cap":"The scratch-ticket glitch.","vo":"Here's a lottery glitch printed right on every single machine.","layout":"mascot"},
   {"cap":"Tickets cost $5 each.","vo":"Each scratch off ticket costs you five dollars.","layout":"icon","icon":"card","num":"$5","col":"red"},
   {"cap":"1 in 4 is a winner.","vo":"The odds on the back say one in four tickets is a guaranteed winner.","layout":"icon","icon":"chart","num":"1 in 4","col":"ink"},
   {"cap":"So buy the WHOLE roll.","vo":"So you don't buy just one ticket, you buy the entire roll at once.","layout":"icons","icon":"card","num":"×300","col":"ink"},
   {"cap":"300 tickets = $1,500.","vo":"A full roll is three hundred tickets for fifteen hundred dollars.","layout":"equation","eq":"300 × $5","num":"= $1,500","col":"red"},
   {"cap":"75 guaranteed winners.","vo":"That means you now hold seventy five guaranteed winning tickets.","layout":"icon","icon":"gift","num":"75 wins","col":"green"},
   {"cap":"They pay out $2,000.","vo":"And those winners pay out around two thousand dollars.","layout":"icon","icon":"cash","num":"$2,000","col":"green"},
   {"cap":"+$500 every roll.","vo":"That's five hundred dollars of profit on every single roll you clear.","layout":"icon","icon":"bag","num":"+$500","col":"green"},
   {"cap":"The odds are printed on it.","vo":"The odds are literally printed on the machine, they basically told you.","layout":"mascot"},
 ]},

 {"id":"matched-betting","title":"Bookies give away free bets — here is how you keep the money","steps":[
   {"cap":"Turn free bets into cash.","vo":"Here's how bookmakers accidentally hand you free money.","layout":"mascot"},
   {"cap":"New users get a $50 free bet.","vo":"Every betting site gives brand new customers a fifty dollar free bet.","layout":"icon","icon":"gift","num":"$50 free","col":"green"},
   {"cap":"Bet $50 on them to WIN.","vo":"You use that free fifty to bet on a team to win.","layout":"icon","icon":"chart","num":"WIN","col":"ink"},
   {"cap":"On another site, bet LOSE.","vo":"Then on a second site, you bet the same fifty on them to lose.","layout":"icon","icon":"phone","num":"LOSE","col":"ink"},
   {"cap":"One side always wins.","vo":"No matter what happens on the field, one of your two bets always wins.","layout":"two","a":"chart","b":"chart"},
   {"cap":"Keep ~$45 free.","vo":"You walk away with about forty five dollars of pure free money.","layout":"icon","icon":"cash","num":"$45","col":"green"},
   {"cap":"Do it on 30 sites.","vo":"Now just repeat that on all thirty different betting sites out there.","layout":"icons","icon":"gift","num":"×30","col":"ink"},
   {"cap":"That's $1,350 risk-free.","vo":"That adds up to over thirteen hundred dollars, completely risk free.","layout":"icon","icon":"bag","num":"$1,350","col":"green"},
   {"cap":"Not gambling if you bet both sides.","vo":"And it isn't really gambling if you bet on both sides at once.","layout":"mascot"},
 ]},

 {"id":"coffee-pass","title":"How a $9 coffee pass secretly pays you $260 a month","steps":[
   {"cap":"Get paid to drink coffee.","vo":"Here's how to make three hundred dollars a month just by drinking coffee.","layout":"mascot"},
   {"cap":"A coffee costs $3.","vo":"A regular coffee shop coffee costs you about three dollars.","layout":"icon","icon":"coffee","num":"$3","col":"red"},
   {"cap":"But there's a $9 unlimited pass.","vo":"But this shop sells an unlimited monthly pass for just nine dollars.","layout":"icon","icon":"card","num":"$9/mo","col":"green"},
   {"cap":"Drink 3 a day.","vo":"So naturally, you drink three whole coffees a day, every single day.","layout":"icons","icon":"coffee","num":"×3/day","col":"ink"},
   {"cap":"90 coffees = $270.","vo":"That is ninety coffees a month, worth two hundred and seventy dollars.","layout":"equation","eq":"90 × $3","num":"= $270","col":"green"},
   {"cap":"You paid $9.","vo":"And you paid a grand total of nine dollars for all of them.","layout":"two","a":"card","b":"coffee"},
   {"cap":"You 'saved' $261.","vo":"So you have technically saved two hundred and sixty one dollars.","layout":"icon","icon":"bag","num":"$261","col":"green"},
   {"cap":"Saving = earning.","vo":"And as everyone knows, saving money is exactly the same as making it.","layout":"icon","icon":"chart"},
   {"cap":"Powered by free espresso.","vo":"You are now a millionaire powered entirely by free espresso.","layout":"mascot"},
 ]},
]
